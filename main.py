...
class FF_CLient:
    ...
    async def STarT(self, JwT_ToKen, AutH_ToKen, ip, port, ip2, port2, key, iv, bot_uid):
        ...
        self._task2 = None
        R = asyncio.Event()
        self._task1 = asyncio.create_task(
        ...
        try:
            ...
            await asyncio.gather(self._task1)
        finally:
            await self._cancel_tasks()
            await self._close_writers()

    async def _cancel_tasks(self):
        for t in (getattr(self, '_task1', None), getattr(self, '_task2', None)):
            if t and not t.done():
                t.cancel()
                try:
    ...
    async def _close_writers(self):
        for attr in ("writer", "writer2"):
            ...
            if w:
                try:
                    w.close()
                    await asyncio.wait_for(w.wait_closed(), timeout=3)
                except Exception:
                    pass
    ...
    async def OnLinE(self, Token, tok, host2, port2, key, iv, bot_uid):
        ...
        while True:
            try:
                self.reader2, self.writer2 = await asyncio.wait_for(asyncio.open_connection(host2, int(port2)), timeout=15)
                self.writer2.write(bytes.fromhex(tok))
                await asyncio.wait_for(self.writer2.drain(), timeout=10)
                while True:
                    try:
                        data = await asyncio.wait_for(self.reader2.read(8192), timeout=30)
                        if not data:
                            break
                        data = None
    ...
    async def ChaT(self, Token, tok, host, port, key, iv, bot_uid, R):
        ...
        while True:
            try:
                self.reader, self.writer = await asyncio.wait_for(asyncio.open_connection(host, int(port)), timeout=15)
...
async def main_async(forced_target, accounts):
    ...
    async def run_client(uid, pwd):
        ...
        try:
            ...
            await asyncio.wait_for(client.run(), timeout=90)
