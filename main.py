    ...
    async def ToKen_GeneRaTe(self, U, P):
        try:
            ...
            try:
                ...
                ResTarTinG()
            self.ResPonse = await asyncio.to_thread(MajorLoGin, self.PaYload)
            if self.ResPonse:
                ...
                ip, port, ip2, port2 = await asyncio.to_thread(
                    self._GeT_LoGin_PorTs_sync, self.JwT_ToKen, self.PaYload
                )
                return (
                    self.JwT_ToKen,
                    self.key,
...
async def main_async(forced_target, accounts):
    ...
    tasks = [run_client(uid, pwd) for uid, pwd in accounts.items()]
...
def StarT_SerVer():
    ...
    asyncio.run(main_async(forced_target, accounts))
